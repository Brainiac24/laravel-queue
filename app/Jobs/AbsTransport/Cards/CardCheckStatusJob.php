...
use App\Jobs\AbsTransport\Cards\CardCheckStatusCallbackJob;
...
class CardCheckStatusJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $this->res_data = $transportService->getCardService()->checkCardStatusRequest($this->data['session_id'], $this->data['card_id']);
            ...
            if ($arr['root']['response']['state'] ?? '0' == '1') {
                ...
                $log_params['Tries']        = $this->tries;
                $log_params['Attempts']     = $this->attempts();
                $log_params['RequestData']  = json_encode($this->data);
                ...
                throw new \Exception("BUS_RESPONSE_STATE_IS_NOT_SUCCESS EO_R_CARDS_LIST");
            }
            ...
            CardCheckStatusCallbackJob::dispatch($dataCallback)->onQueue(QueueEnum::REQUEST);
            throw $conEx;
        } catch (\Throwable $th) {
