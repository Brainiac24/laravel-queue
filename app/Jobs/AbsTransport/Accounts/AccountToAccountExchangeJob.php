...
use App\Jobs\AbsTransport\Accounts\AccountToAccountExchangeCallbackJob;
...
class AccountToAccountExchangeJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $this->res_data = $transportService->getAccountService()->getDocumentService()->createExchangeRequest(
            ...
            if ($arr['root']['response']['state'] ?? '0' == '1' || $arr['root']['response']['state'] ?? '0' == '-2') {
                ...
                AccountToAccountExchangeCallbackJob::dispatch($dataCallback)->onQueue(QueueEnum::PROCESSING);
