...
use App\Jobs\AbsTransport\Accounts\AccountToAccountCallbackV2Job;
...
class AccountToAccountV2Job implements ShouldQueue
{
 ...
 //           $this->logger->info('PARAMS - QUEUE BUS REQUEST --- session_id: '. $this->session_id.' --- class: '. $this->log_name, $log_params);
            ...
            $this->res_data = $transportService->getAccountService()->getDocumentServiceV2()->createTransactionRequest(
            ...
            if ($arr['root']['response']['state'] ?? '0' == '1' || $arr['root']['response']['state'] ?? '0' == '-2') {
                ...
                AccountToAccountCallbackV2Job::dispatch($dataCallback)->onQueue(QueueEnum::PROCESSING);
