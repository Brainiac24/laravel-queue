...
use App\Jobs\AbsTransport\Accounts\AccountToAccountCallbackJob;
...
class AccountToAccountJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $this->res_data = $transportService->getAccountService()->getDocumentService()->createTransactionRequest(
            ...
            if ($arr['root']['response']['state'] ?? '0' == '1' || $arr['root']['response']['state'] ?? '0' == '-2') {
                ...
                AccountToAccountCallbackJob::dispatch($dataCallback)->onQueue(QueueEnum::PROCESSING);
