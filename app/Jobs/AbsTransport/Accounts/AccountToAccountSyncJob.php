...
class AccountToAccountSyncJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $operation_type = DocumentEntity::EO_W_ACCOUNTS_ITEM_TRANSACTIONS_ITEM_CREATE;
            if ($this->data['operation_type']=="EXCHANGE") {
                $operation_type = DocumentEntity::EO_W_ACCOUNTS_ITEM_TRANSACTIONS_ITEM_CREATE_EXCHANGE;
            }

            $this->res_data = $transportService->getAccountService()->getDocumentService()->createRequest(
            ...
            //if ($arr['root']['response']['state'] ?? '0' == '1' || $arr['root']['response']['state'] ?? '0' == '-2') {
                ...
                AccountToAccountSyncCallbackJob::dispatch($dataCallback)->onQueue(QueueEnum::DEFAULT);
