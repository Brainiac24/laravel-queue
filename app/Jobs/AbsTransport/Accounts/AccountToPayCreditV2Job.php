...
use App\Jobs\AbsTransport\Accounts\AccountToPayCreditCallbackV2Job;
...
class AccountToPayCreditV2Job implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $this->res_data = $transportService->getAccountService()->getDocumentServiceV2()->createPayCredit(
            ...
            if ($arr['root']['response']['state'] ?? '0' == '1' || $arr['root']['response']['state'] ?? '0' == '-2') {
                ...
                AccountToPayCreditCallbackV2Job::dispatch($dataCallback)->onQueue(QueueEnum::PROCESSING);
