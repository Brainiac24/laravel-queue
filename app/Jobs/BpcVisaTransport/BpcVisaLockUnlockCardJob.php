...
use App\Jobs\BpcVisaTransport\BpcVisaLockUnlockCardCallbackJob;
use App\Services\Common\Gateway\BpcVisa\BpcVisaCallbackResponse;

class BpcVisaLockUnlockCardJob implements ShouldQueue
{
...
//            'ext_id' => 'required|numeric',
            ...
            'card_status' => 'required|numeric|digits_between:0,1',
        ];
    }
    ...
    public function handle()
    {
        ...
        try {
            ...
            if ($this->data['card_status']==0){
                $this->res_data = $transportService->ChangeCardStatus(
                    $this->data['account'],
//                    Temporary blocked by client 20
                    20
                );
            }elseif($this->data['card_status']==1){
                $this->res_data = $transportService->ValidateCard(
                    $this->data['account']
                );
            }
            ...
            if (is_object($this->res_data) && isset($this->res_data->responseCode) && $this->res_data->responseCode == BpcVisaStatus::OK) {
                ...
                BpcVisaLockUnlockCardCallbackJob::dispatch($dataCallback->getData())->onQueue(QueueEnum::PROCESSING);
