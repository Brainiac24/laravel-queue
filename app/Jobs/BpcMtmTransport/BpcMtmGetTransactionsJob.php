...
use App\Jobs\BpcMtmTransport\BpcMtmGetTransactionsCallbackJob;
use App\Services\Common\Gateway\BpcMtm\BpcMtmCallbackResponse;

class BpcMtmGetTransactionsJob implements ShouldQueue
{
    ...
    public static function rules()
    {
        return [
            ...
            'date_start' => 'required|date_format:Y-m-dH-i-s',
            'date_end' => 'required|date_format:Y-m-dH-i-s',
            'row_start' => 'nullable|numeric',
            'row_end' => 'nullable|numeric',
        ];
    }
    ...
    public function handle()
    {
        ...
        try {
            ...
            $this->res_data = $transportService->GetTransactions(
            ...
            if (count($this->res_data->transactions->transaction)>0) {
                ...
                $dataCallback->history_transactions = json_encode($this->res_data->transactions->transaction,JSON_UNESCAPED_UNICODE);
                $dataCallback->response = json_encode($this->res_data, JSON_UNESCAPED_UNICODE);

                BpcMtmGetTransactionsCallbackJob::dispatch($dataCallback->getData())->onQueue(QueueEnum::PROCESSING);
