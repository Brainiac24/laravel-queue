...
use App\Jobs\RucardTransport\LockUnlockCallbackJob;
...
use App\Services\Common\Gateway\Rucard\Requests\LockUnlockRequest;
...
class LockUnlockJob implements ShouldQueue
{
    ...
    public function handle(Rucard $transport)
    {
        ...
        try {
            ...
            if ($asw['ERRC'] == RucardStatus::OK) {
                ...
                LockUnlockCallbackJob::dispatch($dataResult)->onQueue(QueueEnum::REQUEST);
            } else {
            ...
            LockUnlockCallbackJob::dispatch($dataResult)->onQueue(QueueEnum::PROCESSING);
